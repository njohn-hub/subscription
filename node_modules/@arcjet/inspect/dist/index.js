...
function isBotReason(reason) {
	return typeof reason === "object" && reason !== null && "isBot" in reason && typeof reason.isBot === "function" && reason.isBot() && "isSpoofed" in reason && typeof reason.isSpoofed === "function" && "isVerified" in reason && typeof reason.isVerified === "function";
}
function isErrorReason(reason) {
	return typeof reason === "object" && reason !== null && "isError" in reason && typeof reason.isError === "function" && reason.isError() && "message" in reason && typeof reason.message === "string";
}
function isActive(result) {
	return typeof result === "object" && result !== null && "state" in result && typeof result.state === "string" && result.state !== "DRY_RUN";
}
/**
...
function isSpoofedBot(result) {
	if (isActive(result) && isBotReason(result.reason)) return result.reason.isSpoofed();
}
/**
...
function isVerifiedBot(result) {
	if (isActive(result) && isBotReason(result.reason)) return result.reason.isVerified();
}
/**
...
function isMissingUserAgent(result) {
	if (isActive(result) && isErrorReason(result.reason)) return result.reason.message.includes("missing User-Agent header") || result.reason.message.includes("requires user-agent header");
}
//#endregion
