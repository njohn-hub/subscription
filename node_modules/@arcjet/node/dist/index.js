...
function arcjet(options) {
	...
	function toArcjetRequest(request, props, ipSrc) {
		const cookies = cookiesToString(request.headers?.cookie);
		const headers = new ArcjetHeaders(request.headers);
		const xArcjetIp = isDevelopment(env) ? headers.get("x-arcjet-ip") : void 0;
		let ip = ipSrc || xArcjetIp || findIp({
	...
	function withClient(aj) {
		return Object.freeze({
			...
			async protect(request, props) {
				...
				const req = toArcjetRequest(request, properties || {}, ipSrc);
				const getBody = async () => {
