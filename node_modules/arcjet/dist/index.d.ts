...
import { ArcjetBotCategory, ArcjetBotRule, ArcjetDecision, ArcjetEmailRule, ArcjetEmailType, ArcjetFilterRule, ArcjetFixedWindowRateLimitRule, ArcjetLogger, ArcjetMode, ArcjetPromptInjectionDetectionRule, ArcjetRule, ArcjetSensitiveInfoRule, ArcjetSensitiveInfoType, ArcjetShieldRule, ArcjetSlidingWindowRateLimitRule, ArcjetTokenBucketRateLimitRule, ArcjetWellKnownBot } from "@arcjet/protocol";
 ...
 */
type BotOptions = BotOptionsAllow | BotOptionsDeny;
/**
 ...
 */
type EmailOptions = EmailOptionsAllow | EmailOptionsDeny;
/**
 ...
 */
type SensitiveInfoOptionsAllow<DetectedEntities extends string | undefined = undefined, ListedEntities extends ArcjetSensitiveInfoType | Exclude<DetectedEntities, undefined> = ArcjetSensitiveInfoType | Exclude<DetectedEntities, undefined>> = {
  /**
 ...
 */
type SensitiveInfoOptionsDeny<DetectedEntities extends string | undefined = undefined, ListedEntities extends ArcjetSensitiveInfoType | Exclude<DetectedEntities, undefined> = ArcjetSensitiveInfoType | Exclude<DetectedEntities, undefined>> = {
  /**
 ...
 */
type FilterOptions = FilterOptionsAllow | FilterOptionsDeny;
type PlainObject = {
  [key: string]: unknown;
};
/**
 ...
 *   Configuration.
 */
type Primitive<Props extends PlainObject = {}> = [ArcjetRule<Props>];
/**
 ...
 *   Configuration.
 */
type Product<Props extends PlainObject = {}> = ArcjetRule<Props>[];
type ExcludeBuiltinCharacteristic<T> = T extends "ip.src" | "http.host" | "http.method" | "http.request.uri.path" | `http.request.headers["${string}"]` | `http.request.cookie["${string}"]` | `http.request.uri.args["${string}"]` ? never : [string] extends [T] ? never : T extends string ? T : never;
/**
 ...
 */
type CharacteristicProps<Characteristics extends readonly string[]> = Characteristics extends [] ? {} : { [K in ExcludeBuiltinCharacteristic<Characteristics[number]>]: boolean | number | string; };
 ...
 */
type ExtraProps<Rules> = Rules extends [] ? {} : Rules extends ArcjetRule[][] ? UnionToIntersection<PropsForRule<Rules[number][number]>> : Rules extends ArcjetRule[] ? UnionToIntersection<PropsForRule<Rules[number]>> : never;
/**
 ...
 */
type ArcjetRequest<Props extends PlainObject> = Simplify<{
  /**
 ...
 */
declare function tokenBucket<const Characteristics extends readonly string[] = []>(options: TokenBucketRateLimitOptions<Characteristics>): [ArcjetTokenBucketRateLimitRule<Simplify<UnionToIntersection<{
  requested: number;
 ...
 * @link https://docs.arcjet.com/rate-limiting/reference
 */
declare function fixedWindow<const Characteristics extends readonly string[] = []>(options: FixedWindowRateLimitOptions<Characteristics>): [ArcjetFixedWindowRateLimitRule<CharacteristicProps<Characteristics>>];
/**
 ...
 * @link https://docs.arcjet.com/rate-limiting/reference
 */
declare function slidingWindow<const Characteristics extends readonly string[] = []>(options: SlidingWindowRateLimitOptions<Characteristics>): [ArcjetSlidingWindowRateLimitRule<CharacteristicProps<Characteristics>>];
/**
 ...
 */
declare function validateEmail(options: EmailOptions): [ArcjetEmailRule<{
  email: string;
}>];
/**
 ...
 */
declare function detectBot(options: BotOptions): [ArcjetBotRule<{}>];
/**
 ...
 */
type DetectPromptInjectionOptions = {
  /**
   * Block mode of the rule (default: `"DRY_RUN"`).
   *
   * `"DRY_RUN"` will allow all requests while still providing access to the
 ...
 */
declare function shield(options: ShieldOptions): [ArcjetShieldRule<{}>];
/**
 ...
 *   ```
 */
declare function detectPromptInjection(options?: DetectPromptInjectionOptions): [ArcjetPromptInjectionDetectionRule];
/**
 * Arcjet prompt injection detection rule.
 *
 * @deprecated
 ...
 *   Characteristics to track a user by.
 */
type ProtectSignupOptions<Characteristics extends readonly string[]> = {
  /**
 ...
 */
declare function protectSignup<const Characteristics extends string[] = []>(options: ProtectSignupOptions<Characteristics>): [rateLimit: ArcjetSlidingWindowRateLimitRule<CharacteristicProps<Characteristics>>, bot: ArcjetBotRule<{}>, email: ArcjetEmailRule<{
  email: string;
}>];
/**
 * Arcjet filter rule.
 ...
 */
declare function filter(options: FilterOptions): [ArcjetFilterRule];
/**
 ...
 *   Characteristics to track a user by.
 */
interface ArcjetOptions<Rules extends [...(Primitive | Product)[]], Characteristics extends readonly string[]> {
  /**
 ...
 * should be handled.
 */
interface Arcjet<Props extends PlainObject> {
  /**
 ...
 *   Arcjet instance.
 */
declare function arcjet<const Rules extends [...(Primitive | Product)[]] = [], const Characteristics extends readonly string[] = []>(options: ArcjetOptions<Rules, Characteristics>): Arcjet<ExtraProps<Rules> & CharacteristicProps<Characteristics>>;
//#endregion
export { Arcjet, ArcjetAdapterContext, ArcjetOptions, ArcjetRequest, BotOptions, BotOptionsAllow, BotOptionsDeny, CharacteristicProps, DetectPromptInjectionOptions, EmailOptions, EmailOptionsAllow, EmailOptionsDeny, ExtraProps, FilterOptions, FilterOptionsAllow, FilterOptionsDeny, FixedWindowRateLimitOptions, Primitive, Product, ProtectSignupOptions, SensitiveInfoBackend, SensitiveInfoBackendContext, SensitiveInfoBackendOptions, SensitiveInfoOptions, SensitiveInfoOptionsAllow, SensitiveInfoOptionsDeny, ShieldOptions, SlidingWindowRateLimitOptions, TokenBucketRateLimitOptions, arcjet as default, detectBot, detectPromptInjection, experimental_detectPromptInjection, filter, fixedWindow, protectSignup, sensitiveInfo, shield, slidingWindow, tokenBucket, validateEmail };