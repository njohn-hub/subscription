...
import { ArcjetBotReason, ArcjetDenyDecision, ArcjetEmailReason, ArcjetErrorDecision, ArcjetErrorReason, ArcjetFilterReason, ArcjetPromptInjectionReason, ArcjetRateLimitReason, ArcjetReason, ArcjetRuleResult, ArcjetSensitiveInfoReason, ArcjetShieldReason } from "@arcjet/protocol";
...
function lookupWaitUntil() {
	...
	if (typeof fromSymbol[SYMBOL_FOR_REQ_CONTEXT] === "object" && fromSymbol[SYMBOL_FOR_REQ_CONTEXT] !== null && "get" in fromSymbol[SYMBOL_FOR_REQ_CONTEXT] && typeof fromSymbol[SYMBOL_FOR_REQ_CONTEXT].get === "function") {
		...
		if (typeof vercelCtx === "object" && vercelCtx !== null && "waitUntil" in vercelCtx && typeof vercelCtx.waitUntil === "function") return vercelCtx.waitUntil;
	}
}
/**
...
function toAnalyzeRequest(request) {
	const headers = {};
	if (typeof request.headers !== "undefined") for (const [key, value] of request.headers.entries()) headers[key] = value;
...
function extraProps(details) {
	...
	for (const [key, value] of Object.entries(details)) if (isUnknownRequestProperty(key)) {
...
const slidingWindowValidations = [
	...
	},
	{
		key: "interval",
		required: true,
		validate: validateNumberOrString
	}
];
/**
...
function validateInterface(value, configuration) {
	...
	if (value === null || typeof value !== "object") throw new Error(`\`${name}\` options error: expected object`);
	const valueRecord = value;
	for (const { key, validate, required } of validations) {
		if (required && !Object.hasOwn(valueRecord, key)) throw new Error(`\`${name}\` options error: \`${String(key)}\` is required`);
		const value = valueRecord[key];
		if (typeof value !== "undefined") try {
...
function validateArray(path, value) {
	if (!Array.isArray(value)) throw new Error(`invalid type for \`${path}\` - expected an array`);
}
/**
* Validate a boolean.
*
...
function validateBoolean(path, value) {
	if (typeof value !== "boolean") throw new Error(`invalid type for \`${path}\` - expected boolean`);
}
/**
...
function validateDetails(value) {
	validateInterface(value, {
		name: "details",
		validations: [...knownFields.map(function(key) {
			return {
				key,
				required: key !== "body" && key !== "email" && key !== "correlationId" && key !== "metadata",
				validate: key === "headers" ? validateHeaders : key === "metadata" ? validateMetadata : validateString
			};
		}), {
			key: "extra",
			required: true,
...
function validateHeaders(path, value) {
	if (value === null || typeof value !== "object") throw new Error(`invalid value for \`${path}\` - expected headers object`);
	const methods = [
		"has",
		"get",
		"set"
	...
	const valueRecord = value;
	for (const method of methods) if (!(method in valueRecord) || typeof valueRecord[method] !== "function") throw new Error(`invalid value for \`${path}\` - expected headers object with method \`${method}\``);
}
/**
...
function validateNumberOrString(path, value) {
	if (typeof value !== "number" && typeof value !== "string") throw new Error(`invalid type for \`${path}\` - expected one of string, number`);
}
/**
* Validate a number.
*
...
function validateNumber(path, value) {
	if (typeof value !== "number") throw new Error(`invalid type for \`${path}\` - expected number`);
}
/**
...
function validateStringArray(path, value) {
	...
	for (const [index, subvalue] of value.entries()) validateString(path + "[" + index + "]", subvalue);
}
/**
* Validate a string.
*
...
function validateStringRecord(path, value) {
	if (value === null || typeof value !== "object" || Array.isArray(value)) throw new Error(`invalid value for \`${path}\` - expected plain object`);
	for (const [field, subvalue] of Object.entries(value)) validateString(path + "." + field, subvalue);
}
/**
...
*/
function validateMetadata(path, value) {
	if (!isMetadata(value)) throw new Error(`invalid value for \`${path}\` - expected plain object`);
}
/**
...
function validateString(path, value) {
	if (typeof value !== "string") throw new Error(`invalid type for \`${path}\` - expected string`);
}
/**
* Validate a function.
*
...
function validateFunction(path, value) {
	if (typeof value !== "function") throw new Error(`invalid type for \`${path}\` - expected function`);
}
/**
...
function validateMode(path, value) {
	if (value !== "DRY_RUN" && value !== "LIVE") throw new Error(`invalid value for \`${path}\` - expected one of 'LIVE', 'DRY_RUN'`);
}
/**
...
function validateEmailTypes(path, value) {
	validateArray(path, value);
	for (const [index, subvalue] of value.entries()) if (!isEmailType(subvalue)) throw new Error(`invalid value for \`${path}[${index}]\` - expected one of 'DISPOSABLE', 'FREE', 'NO_MX_RECORDS', 'NO_GRAVATAR', 'INVALID'`);
}
/**
...
*/
function validateSensitiveInfoBackend(path, value) {
	if (typeof value !== "object" || value === null || typeof value.detect !== "function") throw new Error(`invalid type for \`${path}\` - expected an object with a \`detect\` function`);
}
/**
...
function tokenBucket(options) {
	...
	const characteristics = Array.isArray(options.characteristics) ? options.characteristics : void 0;
	...
	return [{
		...
		async protect(context, details) {
			...
			if (cached && cached.reason.isRateLimit()) return new ArcjetRuleResult({
				ruleId,
				fingerprint,
				ttl,
				state: "CACHED",
...
function fixedWindow(options) {
	...
	return [{
		...
		async protect(context, details) {
			const localCharacteristics = characteristics ?? context.characteristics;
			const ruleId = await hasher.hash(hasher.string("type", type), hasher.uint32("version", version), hasher.string("mode", mode), hasher.string("algorithm", "FIXED_WINDOW"), hasher.stringSliceOrdered("characteristics", localCharacteristics), hasher.string("match", ""), hasher.uint32("max", max), hasher.uint32("window", window));
			const analyzeContext = {
				characteristics: localCharacteristics,
				log: context.log
			};
...
function slidingWindow(options) {
	...
	return [{
		...
		async protect(context, details) {
			const localCharacteristics = characteristics ?? context.characteristics;
			const ruleId = await hasher.hash(hasher.string("type", type), hasher.uint32("version", version), hasher.string("mode", mode), hasher.string("algorithm", "SLIDING_WINDOW"), hasher.stringSliceOrdered("characteristics", localCharacteristics), hasher.string("match", ""), hasher.uint32("max", max), hasher.uint32("interval", interval));
			const analyzeContext = {
				characteristics: localCharacteristics,
				log: context.log
			};
...
function protocolSensitiveInfoEntitiesToAnalyze(entity) {
	if (typeof entity !== "string") throw new Error("invalid entity type");
	if (entity === "EMAIL") return { tag: "email" };
	if (entity === "PHONE_NUMBER") return { tag: "phone-number" };
	if (entity === "IP_ADDRESS") return { tag: "ip-address" };
	if (entity === "CREDIT_CARD_NUMBER") return { tag: "credit-card-number" };
	return {
		tag: "custom",
		val: entity
	};
...
*/
const backendOnlySensitiveInfoTypes = /* @__PURE__ */ new Set([
	"GIVEN_NAME",
	"SURNAME",
	"SSN",
	"URL",
...
function sensitiveInfo(options) {
	...
	if (typeof options.allow !== "undefined" && typeof options.deny !== "undefined") throw new Error("`sensitiveInfo` options error: `allow` and `deny` cannot be provided together");
	if (typeof options.allow === "undefined" && typeof options.deny === "undefined") throw new Error("`sensitiveInfo` options error: either `allow` or `deny` must be specified");
	...
	if (typeof options.backend === "undefined" && typeof options.detect === "undefined") {
		const unsupported = [.../* @__PURE__ */ new Set([...allow, ...deny])].filter((entity) => backendOnlySensitiveInfoTypes.has(entity));
		if (unsupported.length > 0) {
			...
			throw new Error(`\`sensitiveInfo\` options error: the ${list} ${subject} only detected when a \`backend\` that supports ${object} is configured (such as \`@arcjet/sensitive-info-rampart\`) or a custom \`detect\` function is provided. The default backend only detects "EMAIL", "PHONE_NUMBER", "IP_ADDRESS", and "CREDIT_CARD_NUMBER".`);
		}
	}
	return [{
		version,
		...
		async protect(context, details) {
			...
			let value;
			if ("sensitiveInfoValue" in details.extra) value = details.extra.sensitiveInfoValue;
			else try {
				...
				return new ArcjetRuleResult({
					...
					state: "NOT_RUN",
					conclusion: "ERROR",
					reason: new ArcjetErrorReason("Cannot read body for sensitive info detection: " + errorMessage(error))
				});
			}
			let convertedDetect = void 0;
			...
			const result = await (options.backend || wasmSensitiveInfoBackend).detect(context, value, entities, {
			...
			if (result.denied.length === 0) return new ArcjetRuleResult({
				ruleId,
				fingerprint,
				ttl: 0,
				state,
			...
			else return new ArcjetRuleResult({
				ruleId,
				fingerprint,
				ttl: 0,
				state,
...
function validateEmail(options) {
	...
	if (typeof options.allow !== "undefined" && typeof options.deny !== "undefined") throw new Error("`validateEmail` options error: `allow` and `deny` cannot be provided together");
	if (typeof options.allow === "undefined" && typeof options.deny === "undefined") throw new Error("`validateEmail` options error: either `allow` or `deny` must be specified");
	const type = "EMAIL";
	const version = 0;
	const mode = options.mode === "LIVE" ? "LIVE" : "DRY_RUN";
	const allow = options.allow ?? [];
	...
	if (typeof options.allow !== "undefined") config = {
	...
	if (typeof options.deny !== "undefined") config = {
		tag: "deny-email-validation-config",
		val: {
			requireTopLevelDomain,
			allowDomainLiteral,
	...
	return [{
		...
		async protect(context, { email }) {
			...
			const state = mode === "LIVE" ? "RUN" : "DRY_RUN";
			if (result.validity === "valid") return new ArcjetRuleResult({
				ruleId,
				fingerprint,
				ttl: 0,
				state,
				...
				state,
				conclusion: "DENY",
				reason: new ArcjetEmailReason({ emailTypes: result.blocked.filter(isEmailType) })
			});
		}
	}];
}
...
function detectBot(options) {
	...
	if (typeof options.allow !== "undefined" && typeof options.deny !== "undefined") throw new Error("`detectBot` options error: `allow` and `deny` cannot be provided together");
	if (typeof options.allow === "undefined" && typeof options.deny === "undefined") throw new Error("`detectBot` options error: either `allow` or `deny` must be specified");
	const type = "BOT";
	const version = 0;
	const mode = options.mode === "LIVE" ? "LIVE" : "DRY_RUN";
	const allow = options.allow ?? [];
	...
	return [{
		...
		validate(_context, details) {
			validateDetails(details);
			if (details.headers.get("user-agent") === null) throw new Error("DetectBot requires `user-agent` header to be set.");
		},
		/**
		...
		*/
		async protect(context, request) {
			...
			const { fingerprint } = context;
			const [cached, ttl] = await context.cache.get(ruleId, fingerprint);
			if (cached) return new ArcjetRuleResult({
				ruleId,
				fingerprint,
				ttl,
				state: "CACHED",
	...
	return [{
		...
		async protect(context, details) {
			...
			return new ArcjetRuleResult({
				...
				state: "NOT_RUN",
				conclusion: "ALLOW",
				reason: new ArcjetShieldReason({ shieldTriggered: false })
			});
		}
	}];
}
...
function detectPromptInjection(options = {}) {
	...
	if (threshold <= 0 || threshold >= 1) throw new Error("`detectPromptInjection` options error: `threshold` must be between 0.0 and 1.0 (exclusive)");
	return [{
		type,
		version,
		threshold,
		...
		validate(_context, details) {
			validateDetails(details);
			if (!("detectPromptInjectionMessage" in details.extra)) throw new Error("detectPromptInjection rule requires `detectPromptInjectionMessage` to be set");
			const message = details.extra.detectPromptInjectionMessage;
			if (typeof message !== "string") throw new Error("detectPromptInjection rule requires `detectPromptInjectionMessage` to be a string");
			if (message.length === 0) throw new Error("detectPromptInjection rule requires `detectPromptInjectionMessage` to be non-empty");
		},
...
function filter(options) {
	...
	const allow = options.allow ?? [];
	const deny = options.deny ?? [];
	if (allow.length > 0 && deny.length > 0) throw new Error("`filter` options error: expressions must be passed in either `allow` or `deny` instead of both");
	if (allow.length === 0 && deny.length === 0) throw new Error("`filter` options error: one or more expressions must be passed in `allow` or `deny`");
	const state = mode === "LIVE" ? "RUN" : "DRY_RUN";
	const type = "FILTER";
	const version = 0;
	...
	return [{
		...
		validate(_context, details) {
			validateDetails(details);
			if (details.extra.filterLocal) validateStringRecord("filterLocal", JSON.parse(details.extra.filterLocal));
		},
		version
	}];
}
...
function arcjet(options) {
	...
	const rt = runtime();
	if (typeof options.log === "undefined") throw new Error("Log is required");
	const log = options.log;
	const perf = new Performance(log);
	if (typeof options.client === "undefined") throw new Error("Client is required");
	...
	async function protect(rules, ctx, request) {
		...
		const details = Object.freeze({
			...
			email: typeof request.email === "string" ? request.email : void 0,
			correlationId: typeof request.correlationId === "string" ? request.correlationId : void 0,
			metadata: isMetadata(request.metadata) ? request.metadata : void 0
		});
		let remoteDetails = {
			...details,
			extra: { ...details.extra }
		};
		for (const field of ["filterLocal", "sensitiveInfoValue"]) if (remoteDetails.extra[field] !== void 0) remoteDetails.extra[field] = "<redacted>";
		remoteDetails = Object.freeze(remoteDetails);
		const reportExtra = { ...remoteDetails.extra };
		if (reportExtra.detectPromptInjectionMessage !== void 0) reportExtra.detectPromptInjectionMessage = "<redacted>";
		...
		try {
			for (const [idx, rule] of rules.entries()) {
				...
				try {
					if (typeof localRule.validate !== "function") throw new Error("rule must have a `validate` function");
					localRule.validate(context, details);
					if (typeof localRule.protect !== "function") throw new Error("rule must have a `protect` function");
					results[idx] = await localRule.protect(context, details);
					if (typeof results[idx] === "undefined") results[idx] = new ArcjetRuleResult({
						ruleId: "",
						fingerprint,
						ttl: 0,
						state: "RUN",
		...
		try {
			...
			if (decision.isDenied() && decision.ttl > 0) {
				...
				for (const result of decision.results) if (result.conclusion === "DENY") cache.set(result.ruleId, result.fingerprint, {
					conclusion: result.conclusion,
					reason: result.reason
				}, result.ttl);
			}
	...
	function createClient(rules) {
		...
		return Object.freeze({
			withRule(rule) {
				return createClient([...rules, ...rule]);
			},
			protect(ctx, request) {
