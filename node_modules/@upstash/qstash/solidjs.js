...
__export(solidjs_exports, {
  serve: () => serve2,
  verifySignatureSolidjs: () => verifySignatureSolidjs
});
...
function assertNonEmptyId(id, label = "id") {
  if (!id) {
...
var ARTIFACTS_LIST_URL = "https://s3.eu-central-1.amazonaws.com/artifacts.upstash.com/?prefix=qstash/versions/&delimiter=/";
...
var fetchLatestVersion = async () => {
  const { ok: ok4, statusCode, body } = await nativeGet(ARTIFACTS_LIST_URL);
  if (!ok4) {
...
var ensureDevelopmentServer = async (env, devMode) => {
...
var Client = class {
  ...
  constructor(config) {
    ...
    if (shouldUseDevelopmentMode(config?.devMode, environment)) {
      ensureDevelopmentServer(environment, config?.devMode).catch((error) => {
